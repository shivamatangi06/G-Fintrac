/* =========================================================
   ...
   Vanilla JavaScript | localStorage | Clean Version
   ========================================================= */

// ===================== CONSTANTS =====================

/** 
 * SAVINGS_TARGET: The goal amount for savings (Total across all months)
 * EMERGENCY_TARGET: The goal amount for the emergency fund (Total across all months)
 */
const SAVINGS_TARGET = 500000;
const EMERGENCY_TARGET = 200000;

/** 
 * DEFAULT_STATE: Initial state structure for new users
 */
const DEFAULT_STATE = {
    transactions: [],
    theme: 'dark',
...
/** 
 * generateUUID: Utility to generate unique IDs for transactions.
 * Uses crypto.randomUUID if available, else falls back to math.random.
 */
...
/** 
 * saveState: Persists the application state to localStorage.
...
/** 
 * loadState: Retrieves the application state from localStorage.
 * Returns default state if no saved data exists or if parsing fails.
 */
function loadState() {
...
/** 
 * switchTab: Handles switching between Dashboard, Insights, and Settings.
 * Updates the active class on nav items and toggles visibility of tab contents.
...
function switchTab(tabId) {
    ...
    // Refresh data corresponding to the active tab
...
/** 
 * isMobile: Checks if the current viewport width is 768px or less.
 * @returns {boolean}
 */
function isMobile() {
...
function toggleSidebar() {
    // SECURITY: Don't allow sidebar to open if the app is currently PIN locked
...
/** 
 * closeSidebar: Specifically closes the sidebar and backdrop.
 */
...
/** 
 * setHamburgerActive: Animates the hamburger icon based on open/closed state.
 * @param {boolean} isActive - True if sidebar is open/expanded.
 */
...
/** 
 * applyTheme: Switches between dark and light themes by updating data-theme attribute.
...
function applyTheme(theme) {
    ...
    // Re-initialize and update charts to match the theme colors
    initCharts();
    updateCharts();
}
...
/** 
 * showToast: Displays a brief notification popup at the bottom of the screen.
...
/** 
 * animateCounter: Animates a numeric value from 0 to target with easing.
...
/** 
 * animateSignedCounter: Similar to animateCounter but handles negative signs.
 * @param {HTMLElement} el - The element to update.
 * @param {number} target - The final value (can be negative).
 */
...
/** 
 * updateMonthLabel: Updates the text content of month labels based on the current state.
 * @param {HTMLElement} labelEl - The label element to update.
 * @param {number} monthIndex - 0-11.
 * @param {number} year - YYYY.
 * @param {boolean} showAll - Whether "All Months" is selected.
 */
...
/** 
 * changeMonth: Cycles to the previous or next month.
 * Also synchronizes the state between Dashboard and Insights views.
...
function changeMonth(direction, context) {
    if (context === 'dash') {
        ...
        // Keep insights in sync with dashboard month change
        ...
        // Keep dashboard in sync with insights month change
        dashShowAll = false;
        dashAllMonths.classList.remove('active');
...
/** 
 * getFilteredTxns: Filters the transaction list by month and year, or returns all.
 * @param {number} monthIndex - 0-11.
 * @param {number} year - YYYY.
 * @param {boolean} showAll - True to ignore month/year filters.
...
/** 
 * computeTotals: Reduces a list of transactions into category-based totals.
 * @param {Array} txns - List of transactions.
 * @returns {Object} - An object containing summed totals for each type.
 */
...
/** 
 * setRingProgress: Updates the SVG progress ring based on percentage.
 * @param {SVGElement} ringEl - The stroke-dasharray element.
 * @param {HTMLElement} pctEl - The text element for percentage.
...
/** 
 * refreshDashboard: Recalculates all dashboard figures and updates the view.
 * This is the main UI update loop for the dashboard tab.
 */
function refreshDashboard() {
    ...
    // Filter transactions for only the currently selected year (for lending/borrowing cards)
    ...
    if (totalNetBalanceEl) {
         ...
         * Income - (Expenses + Emergency + Savings + Lent out)
         * Borrowed amounts are not added to balance as they are liabilities.
         */
        ...
        // Update the button-style pill's border and background based on balance status
        if (netBalCard) {
        ...
        // Color the text specifically (green for surplus, red for deficit)
        totalNetBalanceEl.style.color = netBal >= 0 ? '#34d399' : '#f87171';
    }

    // Update goal progress rings
...
/** 
 * Chart Overlay Plugin: Displays "No data" message when charts are empty.
 */
Chart.register({
    id: 'noDataOverlay',
    afterDraw(chart) {
...
/** 
 * initCharts: Initializes Chart.js instances and configures their styling.
 */
function initCharts() {
...
function updateCharts() {
    ...
    // Group for doughnut chart (by category)
    const catMap = {};
...
/** 
 * renderCategoryChips: Dynamically renders category buttons based on selected transaction type.
 * @param {string} type - Transaction type (income, expense, etc.)
 */
function renderCategoryChips(type) {
    ...
    // Set up click handlers for the newly created chips
...
/** 
 * renderTransactions: Populates the transaction table with rows after sorting and formatting data.
 * @param {Array} txns - The list of transactions to display.
 */
...
/** 
 * refreshInsights: Updates the Insights tab with current period calculations and transaction list.
 */
function refreshInsights() {
    ...
    // Render the list and category bars
...
/** 
 * guessCategory: Smart prediction of transaction category based on description text.
...
/** 
 * parseUploadedFile: Reads and parses CSV/Excel bank statements.
...
function parseUploadedFile(file) {
    ...
    reader.onload = function (e) {
        try {
            ...
            // Logic to convert raw rows into app transaction structure
...
/** 
 * initPinSystem: Resets and shows the PIN lock screen.
 */
...
/** 
 * updatePinDots: Visual feedback for PIN entry (filled/unfilled dots).
 */
...
/** 
 * handlePinKey: Processes keypad or keyboard input for the PIN screen.
 * @param {string} key - The key pressed ('0'-'9' or 'delete').
 */
function handlePinKey(key) {
    ...
    if (pinEntry.length === 4) {
        setTimeout(() => {
            // HARDCODED PIN: 5432
            if (pinEntry === '5432') {
                // If the user was trying to reveal Net Balance, reveal it now.
                if (netBalanceUnlockMode) {
                    netBalanceUnlockMode = false;
                    revealNetBalance();
                    pinLockScreen.style.display = 'none';
                } else {
                    // Otherwise, just unlock the app
                    pinLockScreen.style.display = 'none';
                    initApp();
                }
            } else {
                ...
                if (pinError) {
                    ...
                    void pinError.offsetWidth; // trigger reflow
...
/** 
 * revealNetBalance: Removes the blur effect from the Net Balance value.
 */
...
/** 
 * hideNetBalance: Re-applies the blur effect and resets icons for Net Balance value.
 */
...
/** 
 * initApp: Bootstraps the application after the PIN is verified.
 */
...
// ─── INITIAL PIN CHECK ──────────────────────────────────────────────────
// Shows the lock screen immediately on page load.
initPinSystem();

// ─── RE-LOCK PIN when app comes back from background ────────────────────
// Ensures privacy if the user leaves and then returns to the app.
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
        // Prepare for re-lock when user returns
        ...
        if (sessionStorage.getItem('gf_needs_pin') === '1') {
            ...
            hideNetBalance();       // Blur net balance for privacy
            initPinSystem();        // Show PIN screen again
        }
    }
});
 ...
 * Handles cases where the browser back button returns to the page.
 */
